...
export class DepthSearchMaze {
    ...
    constructor(maze: Cell[][]) {
        this.maze = maze;
        this.start = { x: 1, y: 1 };
        this.end = { x: maze.length - 2, y: maze.length - 2 };
    }
    ...
    private move(actual: Position, previous: Position): Boolean {
        ...
        for (const direction of this.movePositions) {
            if (this.canMove({ x: actual.x + direction.x, y: actual.y + direction.y }, previous)) {

                if (this.move({ x: actual.x + direction.x, y: actual.y + direction.y }, actual)) {
    ...
    private canMove(next: Position, previous: Position): Boolean {
        if (next.x > 0 && next.x < this.maze.length - 1
            && next.y > 0 && next.x < this.maze.length - 1
            && (previous.x != next.x || previous.y != next.y)
            && (!this.maze[next.x][next.y].isWall
                || this.maze[next.x][next.y].isActive)) {
            return true;
        }
        return false;
    }
