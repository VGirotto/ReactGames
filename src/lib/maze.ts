...
export class Maze {
  ...
  private InitializePositions() {
    ...
    // select random start
    do {
      var random_init_x: number = Math.floor(Math.random() * (this.size - 1));
    } while (random_init_x % 2 == 0);

    do {
      var random_init_y: number = Math.floor(Math.random() * (this.size - 1));
    ...
    // select a random end
    do {
      var random_end_x: number = Math.floor(Math.random() * (this.size - 1));
    } while (this.actual.position.x === random_end_x || random_end_x % 2 == 0);

    do {
      var random_end_y: number = Math.floor(Math.random() * (this.size - 1));
  ...
  private CreatePath() {
    ...
    while (!this.actual.active) {
      ...
      do {
        ...
        !next ||
        this.stack.includes(next.position) ||
        this.badPositions.includes(next.position)
      );
  ...
  private Backtrack(countBack: number) {
    // get previous cell
    let pos: Position | undefined = this.stack.at(
      this.stack.length - 2 - countBack
    );
    if (!pos) {
      return [];
    }
